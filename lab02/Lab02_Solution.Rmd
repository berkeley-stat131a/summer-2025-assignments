...
1.  [**dplyr documentation**](https://dplyr.tidyverse.org/) - for altering datasets, e.g. filterout out rows, adding new columns, grouping and aggregating, etc.
2.  [**stringr documentation**](https://stringr.tidyverse.org/)- for editing and manipulating strings
3.  [**forcats documentation**](https://forcats.tidyverse.org/)- for dealing with factors (categorical variables)
4.  [**PingPong LLM**](https://pingpong.hks.harvard.edu/group/62/assistant) (also linked to on the course homepage) - for help when you get stuck - though you can always ask Everett or Huong.
...
```{r}
students = read_csv('data/class_survey_raw.csv')
```
...
renames = c(
    ...
    "What duck do you feel like today?" = "duck",
    "In general, how comfortable (versus shaky) to you feel about writing code?" = "comfort_code",
    "In general, how comfortable (versus shaky) to you feel with statistics, probability, and math stuff?" = "comfort_math",
    "What Data Science (DATA) courses have you taken at Berkeley?" = "courses_data",
    "What Statistics department (STAT) courses have you taken at Berkeley?"  = "courses_stat",     "What programming language are you most familiar with?" = "language",
    "What year are you in your program?" = "year",
    "What is your major?" = "major",
    "What do you think is the best TV series of all time?" = "tv",
    "Fake Test Score" = "fake_score"
)
students = students |>
    rename_with(~ renames[.x], .cols = names(renames))
```
...
```{r}
students = students |>
    mutate(major = fct_lump_min(major, 2, other_level="other"))
```
...
*A. The `class_survey_raw` data you have already loaded into `students` and been working with. Now load the other dataset, `roster.csv` (which is similar to the roster data I get from CalCentral) into a dataframe called `roster`.*
...
*E. Now use `inner_join` to join the two datasets. Same code, just change `full_join` to `inner_join`*
